...
ARC_FindContours::ARC_FindContours(int minArea, int maxArea, bool runApproxPolyDP, bool runConvexHull)
{
...
ARC_FindContours::ARC_FindContours(int minArea, int maxArea, int lowerAngle, int upperAngle, bool runApproxPolyDP, bool runConvexHull)
{
	minContourArea = minArea;
	maxContourArea = maxArea;
...
double ARC_FindContours::dot(Point A, Point B)
{
...
double ARC_FindContours::findMagnitude(Point vector)
{
...
double ARC_FindContours::angleBetween(Point point1, Point vertex, Point point2)
{
...
bool ARC_FindContours::angleTest(vector<Point> contour)
{
	
	if(angleBetween(contour[0],contour[1],contour[2])<lowerAngleThreshold || angleBetween(contour[1],contour[2],contour[3])<lowerAngleThreshold || angleBetween(contour[2],contour[3],contour[0])<lowerAngleThreshold || angleBetween(contour[3],contour[0],contour[1])<lowerAngleThreshold || angleBetween(contour[0],contour[1],contour[2])>upperAngleThreshold || angleBetween(contour[1],contour[2],contour[3])>upperAngleThreshold || angleBetween(contour[2],contour[3],contour[0])>upperAngleThreshold || angleBetween(contour[3],contour[0],contour[1])>upperAngleThreshold)
                            return false;
	return true;
}

//Private - Tests and modifies a given contour with the thresholds and returns if it passed or not
bool ARC_FindContours::applyThresholds(vector<Point> & contour)
{
	if(runHull) 
	{
		vector<Point> hull;
...
void ARC_FindContours::filterContours(vector<vector<Point> > & curContours, vector<vector<Point> > & newContours)
{
...
void ARC_FindContours::getCanny(Mat & image)
{
	Mat imageGray;
...
bool ARC_FindContours::get_contours(Mat image, vector<vector<Point> > & curContours)
{
...
bool ARC_FindContours::get_quads(Mat image, vector<vector<Point> > & curContours)
{
