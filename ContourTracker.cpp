...
//Converts vector<Contour> to vector<vector<Point> > which can then be passed into functions like drawContours
...
//Calculates the angle between two vectors given the vertex and two outside points
...
void findRectangles(Mat image, vector<Contour>  *contours){
			...
			findContours(cannyImage,foundContours,hierarchy,CV_RETR_TREE,CV_CHAIN_APPROX_SIMPLE, Point(0,0));//Finds initial contours

			//Gets the hulls of each contour which estimates straight lines when there is a lot of concavity
			...
			//Checks to see if any of the polygons have angles that are too acute or obtuse and stores the result in a vector of boolean flags
			vector<bool> angles;
			for(int i=0;i<poly.size();i++){
					...
					//cout<<"First: "<<firstAngle<<" Second: "<<secondAngle<<" Third: "<<thirdAngle<<" Fourth: "<<fourthAngle<<endl;
					if(angleBetween(poly[i][0],poly[i][1],poly[i][2])<lowerThreshold || angleBetween(poly[i][1],poly[i][2],poly[i][3])<lowerThreshold ||
						angleBetween(poly[i][2],poly[i][3],poly[i][0])<lowerThreshold || angleBetween(poly[i][3],poly[i][0],poly[i][1])<lowerThreshold
						|| angleBetween(poly[i][0],poly[i][1],poly[i][2])>upperThreshold|| angleBetween(poly[i][1],poly[i][2],poly[i][3])>upperThreshold ||
						angleBetween(poly[i][2],poly[i][3],poly[i][0])>upperThreshold || angleBetween(poly[i][3],poly[i][0],poly[i][1])>upperThreshold)
							angles.push_back(false);
					else angles.push_back(true);
			}
	
			for(int i=0;i<poly.size();i++){
				//If the given hull is a rectangle, large enough to not be an artifact, and meets the angle thresholds, it is added to the return vector
...
/*			if(tracked.size()<=0){
				...
				nomatch.clear();
				findRectangles(image,&tracked);
				for(int z=0;z<tracked.size();z++){
					nomatch.push_back(0);
				}
			}
*/
			//Every tracked contour is checked against every new contour to see if there is a match
			for(int m=0;m<tracked.size();m++){
					
				for(int n=0;n<newContours.size();n++){
				/*	
				...
				*/
					...
					double matchReturn = matchShapes(tracked[m].contour,newContours[n].contour,CV_CONTOURS_MATCH_I1,0);//TODO: Get matchShapes working
					cout<<"matchShapes return: "<<m<<" "<<n<<" "<<matchReturn<<endl;
				
					//Position test - Checks to see if the top left (top right not included for now) corners between two contours are significantly different	
					bool isNear = true;
					...
					if(xDiffTL>positionThreshold || yDiffTL>positionThreshold/* || xDiffBR>positionThreshold || yDiffBR>positionThreshold*/) isNear = false;
			...
			//		waitKey(0);
					...
					//If the contour passes all the tests, it is added to the tracked vector and taken out of the newContours vector
					if(matchReturn<=matchThreshold /*&& isNear==true && areaDifference<=areaThreshold && compare<compareThreshold*/){
							...
							break;//after found, it can stop trying to match that particular tracked contour and move on to match the next tracked contour
					}
					if(n==newContours.size()-1) {
						tracked[m].nomatch++;//if no match for the tracked contour is found after searching all the new contours, increment nomatch
	...
	/*		for(int l=0;l<newContours.size();l++){
			...
			//if a tracked contour can't be found after a certain number of frames, it is kicked out
			for(int m=0;m<tracked.size();m++){
